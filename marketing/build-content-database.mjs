...
function parseDcpPp(html){
  const array=extractBalancedArray(html,'const posts=');
  const items=[]; const re=/\{theme:'([^']*)',hook:(?:'((?:\\.|[^'])*)'|"((?:\\.|[^"])*)"),body:(?:'((?:\\.|[^'])*)'|"((?:\\.|[^"])*)")\}/g;
  let m; const un=s=>String(s??'').replace(/\\n/g,'\n').replace(/\\'/g,"'").replace(/\\"/g,'"');
  while((m=re.exec(array))) items.push({theme:un(m[1]),hook:un(m[2]??m[3]),body:un(m[4]??m[5])});
  return items;
}

function parseStamp(html){
  const array=extractBalancedArray(html,'const raw='); const items=[];
  const re=/\["([^"]+)",`([\s\S]*?)`\]/g; let m;
  while((m=re.exec(array))) items.push({theme:m[1],text:m[2]});
  return items;
}

function utm(product,id,locale){
  const u=new URL(product.destination);
  if(u.hostname==='bit.ly') return product.destination;
  u.searchParams.set('utm_source','threads');u.searchParams.set('utm_medium','social');
  u.searchParams.set('utm_campaign',`${product.campaign}_${locale.toLowerCase()}`);u.searchParams.set('utm_content',id.toLowerCase());return u.toString();
}

const rows=[];
for(const product of config.products){
  const [owner,repo]=product.repository.split('/');
  const url=`https://api.github.com/repos/${owner}/${repo}/contents/${product.sourcePath}?ref=main`;
  const res=await fetch(url,{headers}); if(!res.ok) throw new Error(`${product.repository}: ${res.status}`); const html=await res.text();
  const parsed=product.key==='stamp'?parseStamp(html):parseDcpPp(html);
  if(parsed.length!==50) throw new Error(`${product.name}: expected 50 posts, found ${parsed.length}`);
  parsed.forEach((p,index)=>{
    const baseId=`${product.prefix}-${String(index+1).padStart(3,'0')}`;
    let text=p.text;
    ...
    else text=text.replaceAll('${URL}',product.destination);
...
const cols=['id','base_id','product','language','region','theme','text','cta','destination_url','tracking_url','channel','status','planned_at','published_at','notes'];
